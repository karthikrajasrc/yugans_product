...
const LoginRegister = ({ authModal, setAuthModal }) => {
  ...
  return (
    <div>
      {authModal && (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 mt-15 pt-9 bg-black/60 backdrop-blur-sm animate-[fadeIn_0.2s_ease-out]" onClick={() => setAuthModal(null)}>
        <div className="bg-white rounded-3xl w-full max-w-md overflow-hidden shadow-2xl animate-[slideUp_0.3s_ease-out]" onClick={e=>e.stopPropagation()}>
          <div className="bg-[#1E4A1E] p-5 text-center relative">
            ...
            <div className="w-14 h-14 bg-[#8DC21F] rounded-full flex items-center justify-center mx-auto mb-3 font-black text-white text-2xl shadow-lg">Y</div>
            <h2 className="font-black text-[#8DC21F] text-xl">Yugan's Product</h2>
          ...
          <div className="p-6 space-y-4">
              ...
              <button onClick={handleGoogleLogin} className="flex items-center justify-center gap-3 w-full border border-gray-300 rounded-xl py-2.5 hover:bg-gray-50 transition font-semibold"> <svg width="20" height="20" viewBox="0 0 48 48">
  ...
  <div>
    ...
    <input
      ...
      type="text"
      className="w-full px-4 py-2 rounded-xl border-2 border-[#E8F2D0] bg-[#F5F9E8]"
    />
  </div>
)}
            ...
            <div>
              ...
              <input value={authForm.email} onChange={e => setAuthForm({ ...authForm, email: e.target.value })} placeholder="your@email.com" type="email"
                className="w-full px-4 py-2 rounded-xl border-2 border-[#E8F2D0] bg-[#F5F9E8] text-sm focus:outline-none focus:border-[#8DC21F] transition-colors" />
            </div>
            <div>
            ...
            <button 
              className="w-full bg-linear-to-r from-[#8DC21F] to-[#7AB01A] text-white font-black py-4 rounded-2xl text-sm hover:from-[#A8D832] hover:to-[#8DC21F] transition-all shadow-xl hover:shadow-[#8DC21F]/40 hover:-translate-y-0.5" onClick={authModal === "login" ? handleLogin : handleRegister}>
